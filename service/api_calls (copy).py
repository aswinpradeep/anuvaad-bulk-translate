...
class APICalls:
    def login(self):
        ...
        try:
            req = requests.post(url=ANUVAAD_LOGIN_URL,json=login_body)
            ...
            raise ValueError("error while fetching auth_token : "+str(e))
    ...
    def auth_token_search(self,token):
        ...
        try:
            req = requests.post(url=ANUVAAD_AUTHSEARCH_URL,json=authsearch_body)
            logger.debug(req.status_code)
            if req.status_code >=200 and req.status_code <=204:
            ...
            raise ValueError('error while fetching user_id : '+str(e))
    ...
    def upload_files(self,filepath,token):
        ...
        try:
            ...
            req = requests.post(url=ANUVAAD_UPLOADFILE_URL,files=uploadfiles_body,headers={'auth-token':token})
            logger.debug(req.status_code)
            if req.status_code >=200 and req.status_code <=204:
            ...
            raise ValueError('error while uploading file : '+str(e))
    ...
    def get_status_of_jobs(self,jobs,token):
        ...
        try:
            time.sleep(SLEEP_TIME)
            req = requests.post(url=ANUVAAD_BULK_URL,json=bulk_body,headers={'auth-token':token})
            logger.debug(req.status_code)
            if req.status_code >=200 and req.status_code <=204:
                data = []
            ...
            raise ValueError('error while fetching jobs status : '+str(e))
    ...
    def translate(self,file_name,file_id,token):
        ...
        try:
            time.sleep(SLEEP_TIME)
            req = requests.post(url=ANUVAAD_ASYNCWF_URL,json=asyncwf_body,headers={'auth-token':token})
            logger.debug(req.status_code)
            if req.status_code >=200 and req.status_code <=204:
                resp = req.json()
