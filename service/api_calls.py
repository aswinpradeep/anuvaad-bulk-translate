...
class APICalls:
    def login(self):
        ...
        req = None  # Initialize req to None for later use
        ...
        for retry_count in range(max_retries):
            try:
                ...
                try:
                    ...
                    logger.debug(f"Non-JSON response content: {req.text}")
                ...
                # Log the request exception and retry
                logger.error(f"Retry {retry_count + 1}/{max_retries} failed: {e}")
                time.sleep(retry_delay)

        # Check if the request was successful
        if req and req.status_code >= 200 and req.status_code <= 204:
            try:
                ...
                if token:
                    ...
                    logger.error("Login failed: No token found in the response")
            except ValueError as json_error:
                logger.error(f"Failed to parse JSON to extract token: {json_error}")
                logger.debug(f"Response Text: {req.text}")
        else:
            logger.error(f"Login failed with status code: {req.status_code if req else 'No response'}")
                        ...
                        "files": [
                            {
                                ...
                                "locale": "en",
                                "model": {
                                    "uuid": "c441438c-f4eb-45c3-8e18-972f2d4f6f83",
                                    "is_primary": True,
                                    "model_id": 126,
                                    ...
                                    "target_language_name": "Punjabi",
                                    "description": "AAI4B en-pa model-1(indictrans/fairseq)",
                                    "status": "ACTIVE",
                                    "connection_details": {
                                        "kafka": {
