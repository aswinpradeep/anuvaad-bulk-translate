        ...
        try:
            ...
            for job in status_data:
                if(job["status"] == "FAILED"):
                    wfmrepo.delete_job({"digitization.jobID":job['jobID']})
...
def translate():
    ...
    while True:
        try:
            time.sleep(5)
            result = wfmrepo.search_job({"current_status" : "digitizationCompleted"})
            #Assuming we have digitized jobs
            while len(result)>0 and len(current_jobs_translated) < MAX_TRANSLATION_UPLOADS:
                filename = result[0]["filename"]
                digitized_filename = filename.replace(".pdf","_digitized.pdf")
                logger.debug(f"Uploading file for translation : {digitized_filename}")
                file_id = apicalls.upload_files(os.getcwd()+INPUT_DIRECTORY+digitized_filename,token)
                if file_id is None:
                    continue
                jobID, startTime = apicalls.translate(digitized_filename,file_id,token)
                if jobID is None: 
                    continue
                db_entry = result[0]
...
def cron():
    ...
    while True:
        time.sleep(60*120)
        try:
            result = wfmrepo.search_job({"current_status" : "digitizationProgress"})
            logger.debug(result)
            current_time = round(time.time())
            for each_result in result:
                if len(result) == 0:
                    continue
                logger.debug(f"Cron running for job ID {each_result['digitization']['jobID']}")
                digitization_time = int(str(each_result["digitization"]["startTime"])[:-3])
                current_time = round(time.time())
                dt1 = datetime.datetime.fromtimestamp(digitization_time) # 1973-11-29 22:33:09
                dt2 = datetime.datetime.fromtimestamp(current_time) # 1977-06-07 23:44:50
                rd = dateutil.relativedelta.relativedelta (dt2, dt1)
                print(f"RELATIVE DELTA :: {rd}")
                if rd.days>=1 or rd.hours>=1:
                    logger.debug(f'Job ID : {[each_result["digitization"]["jobID"]]}')
                    status_data = apicalls.get_status_of_jobs([each_result["digitization"]["jobID"]],token)
                    logger.debug(status_data)
                    for job in status_data:
                        if(job["status"] == "INPROGRESS" and (rd.days >= 1 or rd.hours >= 6)):
                            wfmrepo.delete_job({"digitization.jobID":job['jobID']})
                        if(job["status"] == "FAILED"):
                            wfmrepo.delete_job({"digitization.jobID":job['jobID']})
                        if(job["status"] == "COMPLETED"):
                            ...
                            document = apicalls.document_export(user_id, record_id, token)
                            file_content = apicalls.download_file(document, token)
                            digitized_file = "input/"+each_result["filename"].replace(".pdf","")+"_digitized.pdf"
                            with open(digitized_file, "wb") as file:
                                file.write(file_content)

                            digitized_copy = "digitized/"+each_result["filename"].replace(".pdf","")+"_digitized.pdf"

                            with open(digitized_copy, "wb") as file:
                                file.write(file_content)
                                
                            wfmrepo.update_job(each_result["filename"],{"current_status" : "digitizationCompleted"})
